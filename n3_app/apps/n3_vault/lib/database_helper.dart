...
class DatabaseHelper {
  ...
  Future<Database> _initDB() async {
    try {
      ...
      final path = join(dbPath, 'n3_vault.db');
  ...
  Future<void> _copyFromAssets(String path) async {
    ...
    try {
      ...
      ByteData data = await rootBundle.load(join('assets', 'n3_vault.db'));
      List<int> bytes =
  ...
  Future<Map<String, int>> getSRSStats(bool isVocab) async {
    ...
    try {
      ...
      int totalVault = 0;
      final totalRecords = await db.rawQuery(isVocab
          ? "SELECT COUNT(*) as count FROM $contentTable"
          : "SELECT COUNT(*) as count FROM $contentTable WHERE audit_status = 'verified'");
      if (totalRecords.isNotEmpty) {
        totalVault = (totalRecords.first['count'] as num?)?.toInt() ?? 0;
      }

      int studied = 0;
      final studiedRecords = await db.rawQuery("SELECT COUNT(*) as count FROM $progressTable");
      ...
      String studiedKey = isVocab ? 'vocab_new_cards_studied_today' : 'grammar_new_cards_studied_today';
      ...
      if (isVocab) {
        final sentenceTotal = await db.rawQuery("SELECT COUNT(*) as count FROM example_sentences WHERE audit_status = 'verified'");
        if (sentenceTotal.isNotEmpty) {
          displayTotal = (sentenceTotal.first['count'] as num?)?.toInt() ?? 0;
        }

        final sentenceStudied = await db.rawQuery("SELECT COUNT(*) as count FROM example_sentences WHERE audit_status = 'verified' AND vocab_id IN (SELECT vocab_id FROM $progressTable)");
  ...
  Map<String, String> getSRSIntervals(Map<String, dynamic>? progress) {
    if (progress == null || (progress['state'] ?? 'new') == 'new' || progress['state'] == 'learning' || progress['state'] == 'relearning') {
  ...
  Future<void> updateSRSProgress(bool isVocab, int id, String rating) async {
    ...
    if (state == 'new' || state == 'learning' || state == 'relearning') {
      ...
      switch (rating.toLowerCase()) {
        ...
        case 'hard':
          if (repetitions == 0) {
            nextReview = now.add(const Duration(minutes: 6)); // Average of 1m and 10m
          } else {
            nextReview = now.add(const Duration(minutes: 10)); // Repeat the 10m step
          }
          state = 'learning';
          break;
        case 'good':
          ...
          break;
        case 'easy':
          nextReview = now.add(const Duration(days: 4)); // Immediate Graduation (Easy)
          interval = 4;
          state = 'review';
          break;
      }
