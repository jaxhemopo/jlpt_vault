...
func main() {
	...
	// 2. Create the SQLite file (The Asset) for N1 Vault
	cwd, _ := os.Getwd()
	var dbPath string
	if filepath.Base(cwd) == "exporter" {
		dbPath = "../../apps/n1_vault/assets/n1_vault.db"
	} else {
		dbPath = "apps/n1_vault/assets/n1_vault.db"
	}
	...
	fmt.Println("🏗️  N1 SQLITE EXPORT (jlpt_level=1 vocab + N1 grammar only)...")
	...
	vRows, err := pgDB.Query("SELECT id, kanji, reading, english_meaning, category, jlpt_level FROM vocabulary WHERE jlpt_level = 1")
	if err != nil {
	...
	sRows, err := pgDB.Query(`
		...
		WHERE v.jlpt_level = 1 AND es.audit_status = 'verified'`)
	if err != nil {
	...
	gRulesRows, err := pgDB.Query(`
		...
		WHERE COALESCE(NULLIF(TRIM(grammar_level), ''), 'N1') = 'N1'`)
	if err != nil {
