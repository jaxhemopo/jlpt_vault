...
func main() {
	...
	// Assuming .env is in the root of your n1_app workspace
	...
	query := `
		...
		AND COALESCE(NULLIF(TRIM(gr.grammar_level), ''), 'N1') = 'N1'
	`
...
func generateEnglishTranslation(apiKey, japaneseSentence, grammarRule string) (string, error) {
	systemPrompt := `
    You are an expert JLPT N1 Japanese-to-English translator.
