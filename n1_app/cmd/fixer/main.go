...
func main() {
	...
	fmt.Println("🛠️ N1 VOCAB FIXER (jlpt_level=1 only, iron-clad headword retention)...")
	...
	for {
		...
		prompt := fmt.Sprintf(`Act as a strict JLPT N1 editor.
        ...
        STYLE (same as generator): Daily line ~N2 grammar; formal line ~N1. Natural everyday / study / travel / media contexts — avoid corporate-default tone unless the word demands it.

        FORMATTING:
        - Furigana (N1 policy): optional on common kanji; add 漢字[よみ] for harder/ambiguous kanji and for the target headword when it uses kanji. No brackets on katakana-only words.
		...
		resp, err := client.CreateChatCompletion(
			...
			openai.ChatCompletionRequest{
				...
				Messages: []openai.ChatCompletionMessage{
					{Role: openai.ChatMessageRoleSystem, Content: "Return JSON: {\"sentence_jp\": \"...\", \"sentence_en\": \"...\"}. Preserve the target headword. Furigana optional on common kanji; add readings where helpful for N1. N1-appropriate Japanese (daily ~N2 / formal ~N1). English must match the new Japanese."},
