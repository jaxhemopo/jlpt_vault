...
func main() {
	...
	fmt.Println("🔎 N1 VOCAB AUDITOR (jlpt_level=1 only, throttled)...")
	...
	for {
		...
		prompt := fmt.Sprintf(`Audit these JLPT N1 vocabulary sentences (same policy as the N1 generator + fixer).
		...
		2) Style: Appropriate for N1 study — daily line ~N2-clear; formal line ~N1. Flag stiff corporate/business-default tone unless the headword requires it.
		3) Furigana: N1 learners — omitting ruby on common kanji is acceptable when reading is obvious at this level. Readings in brackets should be correct when present; flag wrong readings or brackets on katakana-only words. Prefer brackets on the target headword when it uses kanji.
		...
		resp, err := client.CreateChatCompletion(
			...
			openai.ChatCompletionRequest{
				...
				Messages: []openai.ChatCompletionMessage{
					{Role: openai.ChatMessageRoleSystem, Content: "Return JSON with a results array. Each item: id, status ('verified' or 'flagged' only), comment. Judge target usage, translation quality, N1-appropriate tone. Furigana: do not require full ruby on every kanji — only flag incorrect readings or bad bracket usage."},
