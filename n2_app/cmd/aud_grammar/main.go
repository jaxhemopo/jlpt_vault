...
func main() {
	...
	fmt.Println("🔍 N2 GRAMMAR AUDITOR: grammar_level=N2 cards, batch 5")
						...
						Messages: []openai.ChatCompletionMessage{
							{
								...
								Content: "You are a strict JLPT N2 editor. Reply ONLY 'valid' or 'invalid'.",
							},
							{
								Role: openai.ChatMessageRoleUser,
								Content: fmt.Sprintf(`Check this JLPT N2 grammar card (N2 furigana policy: full ruby not required on common kanji).
...
3) Japanese: natural N2 level; not unnecessarily business/office-heavy.
...
Reply valid or invalid only.`, ruleName, ruleMeaning, sJP, cAns),
		...
		if found {
			...
			db.QueryRow(`
				...
				JOIN grammar_rules r ON r.id = c.grammar_id
				WHERE c.audit_status = 'pending'
				AND COALESCE(NULLIF(TRIM(r.grammar_level), ''), 'N2') = 'N2'`).Scan(&pendingCount)
			if pendingCount == 0 {
