...
func main() {
	...
	fmt.Println("🛠️ N2 GRAMMAR FIXER: grammar_level=N2 flagged cards")
	...
	for {
		...
		for attempt := 1; attempt <= 3; attempt++ {
			...
			//
			prompt := fmt.Sprintf(`Fix this grammar drill card for a JLPT N2 app (furigana optional on common N4–N5 kanji).
...
TASK:
1) sentence_jp — natural Japanese at N2 level (not corporate-default). Must use the grammar pattern.
2) Furigana: add 漢字[よみ] only where it helps N2 learners; plain kanji OK for common vocabulary. No brackets on katakana.
			...
			resp, err := client.CreateChatCompletion(
				...
				openai.ChatCompletionRequest{
					...
					Messages: []openai.ChatCompletionMessage{
						{Role: openai.ChatMessageRoleSystem, Content: "Return JSON only: sentence_jp, sentence_en, cloze_sentence_jp, cloze_answer. N2 furigana policy (optional on common kanji); one [____]; cloze must splice exactly."},
