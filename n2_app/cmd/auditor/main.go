...
func main() {
	...
	fmt.Println("🔎 N2 VOCAB AUDITOR (jlpt_level=2 only, throttled)...")
	...
	for {
		...
		prompt := fmt.Sprintf(`Audit these JLPT N2 vocabulary sentences (same policy as the N2 generator + fixer).
		...
		2) Style: Appropriate for N2 study — daily line ~N3-clear; formal line ~N2. Flag stiff corporate/business-default tone unless the headword requires it.
		3) Furigana: N2 learners — omitting ruby on common N4–N5 kanji is acceptable. Readings in brackets should be correct when present; flag wrong readings or brackets on katakana-only words. Prefer brackets on the target headword when it uses kanji.
		...
		resp, err := client.CreateChatCompletion(
			...
			openai.ChatCompletionRequest{
				...
				Messages: []openai.ChatCompletionMessage{
					{Role: openai.ChatMessageRoleSystem, Content: "Return JSON with a results array. Each item: id, status ('verified' or 'flagged' only), comment. Judge target usage, translation quality, N2-appropriate tone. Furigana: do not require full ruby on every kanji — only flag incorrect readings or bad bracket usage."},
