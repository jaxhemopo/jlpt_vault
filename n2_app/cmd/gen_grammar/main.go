...
func main() {
	...
	fmt.Println("🧠 N2 GRAMMAR GENERATOR: N2 rules only (5 cards); furigana optional on common kanji")
	...
	for {
		...
		err := db.QueryRow(`
			SELECT r.id, r.name, r.structure, r.meaning, COALESCE(NULLIF(TRIM(r.grammar_level), ''), 'N2')
		...
		prompt := fmt.Sprintf(`You create ONE grammar drill card for JLPT N2 learners.

RULE NAME: %s
STRUCTURE / PATTERN (use this in the sentence): %s
MEANING: %s
LEVEL: %s
SCENE (loose hint only): %s — natural, everyday / study / travel Japanese at N2 level. Do NOT default to business, office, or heavy keigo.
...
FURIGANA (N2 learners — not full-ruby like N4/N5):
- Optional on common N4–N5-level kanji. Add 漢字[よみ] for harder / ambiguous characters or where it helps the learner.
...
Furigana: N2 policy — optional on common kanji; add readings where helpful. No brackets on katakana.
Cloze: one "[____]" only; cloze_answer must splice back to sentence_jp exactly.
Style: natural JLPT N2-level Japanese — not corporate-default.`,
						},
