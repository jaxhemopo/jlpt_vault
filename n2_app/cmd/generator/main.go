...
func main() {
	...
	fmt.Println("📢 N2 GENERATOR: jlpt_level=2 — daily ~N3 / formal ~N2; furigana optional on common kanji")
	...
	for {
		...
		for attempts := 1; attempts <= 3; attempts++ {
			...
			prompt := fmt.Sprintf(`Act as a JLPT N2 teacher (this row is JLPT N2 vocabulary).
            ...
            Category "%s" is a loose theme only: prefer natural everyday life, study, travel, and social situations. Do NOT default to business, office, or heavy keigo unless the headword itself is clearly workplace-specific.
            %s
            Output: exactly 2 Japanese sentences + 2 English translations.
            1) daily_sentence + english_daily: natural Japanese with roughly N3-level grammar — clear and conversational.
            2) formal_sentence + english_formal: natural Japanese with N2-level grammar and register (polite / written-style where appropriate), still learner-appropriate.

            If the headword starts with ～, that marks a slot: omit ～ in the sentence; fill naturally (e.g. 新宿 for a place + 区 for ～区). Add readings in brackets only where they help an N2 learner (uncommon kanji, ambiguous readings, or the target headword when it uses kanji).

            FURIGANA POLICY (N2 learners — not graded like N4/N5):
            - Do NOT feel obliged to put ruby on every common N4/N5-level kanji (e.g. 私、今日、学校、食べる-style vocabulary). Plain kanji is fine when the reading is standard at this level.
            ...
            - Keep sentences natural for JLPT N2 study — quality of Japanese and headword usage matters more than full ruby coverage.`, cleanK, cleanR, category, kanaOnlyNote)
					...
					Messages: []openai.ChatCompletionMessage{
						{
                            ...
                            Style: JLPT N2 vocabulary study — daily line ~N3 grammar, formal line ~N2 grammar; natural contexts, not corporate-default.
                            Furigana: optional on common N4–N5 kanji; add 漢字[よみ] for harder or ambiguous kanji and for the target headword when it uses kanji. No brackets on katakana-only words.
			...
			_, err = db.Exec(`
                    ...
                    VALUES ($1, $2, $3, 'daily', 'N3', 'pending'), ($1, $4, $5, 'formal', 'N2', 'pending')`,
