...
func main() {
	...
	for {
		...
		err := db.QueryRow(`
			...
			WHERE c.audit_status = 'flagged' LIMIT 1`).Scan(&id, &rule, &structure, &meaning, &sJP)
		...
		for attempt := 1; attempt <= 3; attempt++ {
			...
			//
			prompt := fmt.Sprintf(`Fix this grammar drill card for a JLPT N4/N5 app (same furigana rules as vocabulary sentences).
...
TASK:
1) sentence_jp — natural, simple everyday Japanese (not business/office default). Must use the grammar pattern.
2) Furigana: after each kanji cluster, [hiragana] before any later kanji; okurigana before [ OK (行[い]きます). Katakana: no brackets. Same mechanical rules as 私[わたし]は今日[きょう]…
			...
			resp, err := client.CreateChatCompletion(
				...
				openai.ChatCompletionRequest{
					...
					Messages: []openai.ChatCompletionMessage{
						{Role: openai.ChatMessageRoleSystem, Content: "Return JSON only: sentence_jp, sentence_en, cloze_sentence_jp, cloze_answer. N4/N5 learner furigana; one [____]; cloze must splice exactly."},
