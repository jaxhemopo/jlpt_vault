...
func main() {
	...
	// 2. Create the SQLite file (The Asset) for N4/N5 Vault
	cwd, _ := os.Getwd()
	var dbPath string
	if filepath.Base(cwd) == "exporter" {
		dbPath = "../../apps/n45_vault/assets/n45_vault.db"
	} else {
		dbPath = "apps/n45_vault/assets/n45_vault.db"
	}
	...
	fmt.Println("🏗️  BUILDING UNIFIED SQLITE EXPORT (VOCAB + GRAMMAR)...")
	...
	_, err = sqliteDB.Exec(`
		...
		CREATE TABLE grammar_rules (
			...
			structure TEXT,
			meaning TEXT,
			grammar_level TEXT DEFAULT 'N4/N5'
		);
	...
	vRows, err := pgDB.Query("SELECT id, kanji, reading, english_meaning, category, jlpt_level FROM vocabulary")
	if err != nil {
	...
	sRows, err := pgDB.Query("SELECT id, vocab_id, sentence_jp, sentence_en, cloze_deletion_index, audit_status FROM example_sentences WHERE audit_status = 'verified'")
	if err != nil {
	...
	gRulesRows, err := pgDB.Query("SELECT id, name, structure, meaning, grammar_level FROM grammar_rules")
	if err != nil {
	...
	gCardRows, err := pgDB.Query("SELECT id, grammar_id, sentence_jp, sentence_en, cloze_sentence_jp, cloze_answer, audit_status FROM grammar_cards WHERE audit_status = 'verified'")
	if err != nil {
