...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  bool _includeN4 = true;
  bool _includeN5 = true;
  ...
  Future<void> _loadStats() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _includeN4 = prefs.getBool('include_n4_cards') ?? true;
        _includeN5 = prefs.getBool('include_n5_cards') ?? true;
        _isLoading = false;
      });
    }
  }

  Future<void> _updateLevelToggle({
    required bool isN4,
    required bool value,
  }) async {
    final prefs = await SharedPreferences.getInstance();
    final nextN4 = isN4 ? value : _includeN4;
    final nextN5 = isN4 ? _includeN5 : value;
    if (!nextN4 && !nextN5) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('At least one level must stay enabled.'),
          ),
        );
      }
      return;
    ...
    await prefs.setBool(isN4 ? 'include_n4_cards' : 'include_n5_cards', value);
    if (mounted) {
      setState(() {
        _includeN4 = nextN4;
        _includeN5 = nextN5;
                    ...
                    ? () async {
                        ...
                        if (mounted) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Study progress has been completely reset.',
                              ),
                            ),
                          );
                        }
            ...
            children: [
                ...
                child: Column(
                  children: [
                    ...
                    SwitchListTile(
                      value: _includeN4,
                      onChanged: (val) =>
                          _updateLevelToggle(isN4: true, value: val),
                      secondary: Icon(
                        Icons.filter_4,
                      ...
                      subtitle: Text(
                        "Study N4 vocab and grammar in sessions",
                    ...
                    SwitchListTile(
                      value: _includeN5,
                      onChanged: (val) =>
                          _updateLevelToggle(isN4: false, value: val),
                      secondary: Icon(
                        Icons.filter_5,
                        color: Theme.of(context).colorScheme.primary,
                      ),
                      ...
                      subtitle: Text(
                        "Study N5 vocab and grammar in sessions",
                        style: TextStyle(color: subColor, fontSize: 12),
                      ),
                      contentPadding: const EdgeInsets.symmetric(
                        horizontal: 20,
                  ...
                  children: [
                    SwitchListTile(
                      ...
                      onChanged: (val) async {
                        final prefs = await SharedPreferences.getInstance();
                        if (val) {
                          bool granted = await NotificationManager()
                              .requestPermissions();
                          if (!granted) {
                            if (mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      subtitle: Text(
                        "Get reminded when cards are ready",
                        style: TextStyle(color: subColor, fontSize: 12),
                      ),
                      contentPadding: const EdgeInsets.symmetric(
                        horizontal: 20,
                  ...
                  children: [
                    ...
                    if (_isUnlocked) ...[
                      ListTile(
                        ...
                        subtitle: Text(
                          "Manage your purchase or subscription",
                    ...
                    ListTile(
                      ...
                      subtitle: Text(
                        "Check for existing App Store licenses",
                        style: TextStyle(color: subColor, fontSize: 11),
                      ),
                      trailing: Icon(
                        Icons.chevron_right,
              ...
              _buildSettingsCard(
                child: ListTile(
                  ...
                  subtitle: Text(
                    "Delete timeline and restart vault",
                    style: TextStyle(color: subColor, fontSize: 12),
                  ),
                  trailing: Icon(
                    Icons.chevron_right,
                  ...
                  children: [
                    ...
                    ListTile(
                      ...
                      subtitle: Text(
                        _isLoading
                            ? "Loading..."
                            : "Vocab: $_vocabTotal | Grammar: $_grammarTotal",
                        style: TextStyle(color: subColor, fontSize: 12),
                      ),
                      trailing: Icon(
                        Icons.check_circle,
                    ...
                    ListTile(
                      ...
                      subtitle: Text(
                        "Legal agreements and data safety",
                        style: TextStyle(color: subColor, fontSize: 11),
                      ),
                      trailing: Icon(
                        Icons.open_in_new,
                      ...
                      onTap: () async {
                        final url = Uri.parse(
                          'https://kaji-hemopo.github.io/n45-vault-web/',
                        );
