...
class HomeScreen extends StatefulWidget {
  ...
  const HomeScreen({super.key, this.onBackToLevels});
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    DatabaseHelper.activeLevelRevision.addListener(_onLevelOrDbChanged);
    _refreshStats();
  }

  @override
  void dispose() {
    DatabaseHelper.activeLevelRevision.removeListener(_onLevelOrDbChanged);
    super.dispose();
  }
      ...
      children: [
        Column(
          ...
          children: [
            ...
            if (DatabaseHelper().selectedLevel != null) ...[
              const SizedBox(height: 2),
              Text(
                'N${DatabaseHelper().selectedLevel}',
                style: TextStyle(
                  color: AppColors.subText(context),
                  fontSize: 13,
  ...
  Widget _buildStartButton() {
    return Material(
      ...
      child: InkWell(
        onTap: () async {
          final canStart = await _canStartStudy();
  ...
  Future<bool> _canStartStudy() async {
    ...
    final used = (lastDate == today) ? (prefs.getInt(_dailyFreeCardsUsedKey) ?? 0) : 0;
    if (used < _dailyFreeCardLimit) return true;
    if (!mounted) return false;

    if (!IapManager().isRevenueCatEnabled) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Daily free limit reached (10 cards).'),
        ),
      );
      return false;
    }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildTutorialStep(
                    '4. Reading Support',
                    'Tap the "ABC" toggle in the study arena to reveal furigana for N3+ level Kanji. Note: Kanji expected for N3 level will remain hidden to challenge your memory!',
                  ),
                ],
              ),
            ),
