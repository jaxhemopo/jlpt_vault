...
class _StudyArenaState extends State<StudyArena> with SingleTickerProviderStateMixin {
  ...
  void _nextCard(String rating) async {
    ...
    if (_currentIndex < _cards.length) {
      ...
      final hitFreeLimit = await _handleDailyFreeLimitAfterCard();
      if (hitFreeLimit) {
  ...
  Future<bool> _handleDailyFreeLimitAfterCard() async {
    final hasAccess = await IapManager().hasActiveAccess();
    if (hasAccess) return false;

    final prefs = await SharedPreferences.getInstance();
    final today = DateTime.now().toIso8601String().split('T')[0];
    final lastDate = prefs.getString(_dailyFreeCardsDateKey) ?? '';
    int used = prefs.getInt(_dailyFreeCardsUsedKey) ?? 0;

    if (lastDate != today) {
      used = 0;
      await prefs.setString(_dailyFreeCardsDateKey, today);
    }

    used += 1;
    await prefs.setInt(_dailyFreeCardsUsedKey, used);

    if (used < _dailyFreeCardLimit) return false;
    if (_paywallShownThisSession || !mounted) return true;
    ...
    try {
      ...
      final offering = offerings.all["Vault Access"];
      await RevenueCatUI.presentPaywall(offering: offering);
    } catch (e) {
      print("CARD_PAYWALL_ERROR: $e");
  ...
  void _showFlagCardDialog() {
    ...
    final card = _cards[_currentIndex];
    final level = DatabaseHelper().selectedLevel;
    final levelLabel = level != null ? 'N$level' : 'Unknown';
            ...
            onPressed: () async {
              ...
              final Uri emailLaunchUri = Uri(
                ...
                query: encodeQueryParameters(<String, String>{
                  'subject': 'JLPT Vault [$levelLabel] card flagged',
                  'body': 'Card: $cardInfo\n\nIssue:\n$userMsg',
                }),
              );
  ...
  Widget _buildVocabWord(
    Map<String, dynamic> card, {
    required double fontSize,
    required Color color,
    double letterSpacing = 0,
  }) {
    final bool isN5 = DatabaseHelper().selectedLevel == 5;
    final String kanji = (card['kanji']?.toString() ?? '').trim();
    final String reading = (card['reading']?.toString() ?? '').trim();
    final String word = kanji.isNotEmpty ? kanji : reading;
    final bool canShowFurigana = isN5 && reading.isNotEmpty && reading != word;
    final String furiganaText = canShowFurigana ? '$word($reading)' : word;
